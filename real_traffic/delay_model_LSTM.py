...
"""
See real_traffic/delay_model.py for more details.
The architecture is the same, except it uses LSTM cells instead of GRU cells.
For modeling flow-wise dependencies on both links and queues.
"""
...
class RouteNet_Fermi(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        h_queue_state = queue_state



        c_queue_state = tf.zeros_like(h_queue_state)
        c_path_state = tf.zeros_like(h_path_state)
        c_link_state = tf.zeros_like(h_link_state)


        for it in range(self.iterations):


            queue_gather = tf.gather(h_queue_state, queue_to_path)
            link_gather = tf.gather(h_link_state, link_to_path, name="LinkToPath")
            path_update_rnn = tf.keras.layers.RNN(self.path_update,
                                                  return_sequences=True,
                                                  return_state=True)

            previous_h_path_state = h_path_state

            path_state_sequence, h_path_state, c_path_state = path_update_rnn(
                tf.concat([queue_gather, link_gather], axis=2),
                initial_state=[h_path_state, c_path_state]
            )

            path_state_sequence = tf.concat([tf.expand_dims(previous_h_path_state, 1), path_state_sequence], axis=1)


            path_gather = tf.gather_nd(path_state_sequence, path_to_queue)
            path_sum = tf.math.reduce_sum(path_gather, axis=1)
            queue_state_output, [h_queue_state, c_queue_state] = self.queue_update(
                path_sum, states=[h_queue_state, c_queue_state]
            )


            queue_gather = tf.gather(h_queue_state, queue_to_link)
            link_rnn = tf.keras.layers.RNN(self.link_update, return_sequences=False, return_state=True)
            link_state_output, h_link_state, c_link_state = link_rnn(
                queue_gather, initial_state=[h_link_state, c_link_state]
            )
